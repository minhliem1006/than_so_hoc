...
const handleLogic = ()=>{
  ...
  let dataFullName = document.querySelectorAll('input[name="inputName"]')
  // console.log(dataFull);
  let dataSubmit = {
    name:[],
    day:[],
  }
  dataFullName.forEach((item, index) => {
      dataSubmit.name.push(item.value)
  })
  dataFullDay.forEach((item, index) => {
    dataSubmit.day.push(item.value)
  })
    setFullName(dataSubmit.name)
    setFullDay(dataSubmit.day)
}
const hanleSubmit = (e) => {
  e.preventDefault();
  handleLogic();
